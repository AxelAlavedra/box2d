...
PhysBody* ModulePhysics::CreateCircle(int x, int y, int radius, Module* mod)
{
	b2BodyDef body;
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	bool hit = false;
	for (b2Fixture* f = body->GetFixtureList(); f && hit == false; f = f->GetNext()) {
		hit = f->RayCast(&output, input, childIndex);
		if (hit)
		{
			b2Vec2 hitPoint = input.p1 + output.fraction * (input.p2 - input.p1);
			normal_x = METERS_TO_PIXELS(output.normal.x);
			normal_y = METERS_TO_PIXELS(output.normal.y);
			ret = METERS_TO_PIXELS(sqrt(((input.p1.x * input.p1.x) - (input.p2.x * input.p2.x)) + ((input.p1.y*input.p1.y) - (input.p2.y*input.p2.y))));
		}
	}
...
void ModulePhysics::BeginContact(b2Contact * contact)
{
	...
	if (pb1->mod != nullptr) {
		pb1->mod->OnCollision(pb1, pb2);
	}
	if (pb2->mod != nullptr) {
		pb2->mod->OnCollision(pb1, pb2);
	}
}
// TODO 3
